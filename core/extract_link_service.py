...
def _runtime_setting(name: str, default=None, *, preserve_empty: bool = False):
    """
    提链配置多数保存在 .env。服务模块会在 WebUI 启动时较早 import，
    因此每次实际读取时都重新加载 .env，避免“页面已保存但当前进程仍读到空值”。
    ``preserve_empty`` 用于区分“明确清空的环境变量”和未配置，和 DJB
    客户端的运行时配置读取约定保持一致。
    """
    try:
...
def _djb_item_for_email(snapshot: dict, email: str) -> dict:
    ...
    items = djb_client.task_items(snapshot)
    target = str(email or "").strip().lower()
    for item in items:
        if str(item.get("email") or "").strip().lower() == target:
            return item
    # A single anonymous item is safe to associate with this one-account task;
    # a non-empty mismatched email must never be treated as our result.
    if len(items) == 1 and not str(items[0].get("email") or "").strip():
        return items[0]
    return {}
...
def _run_djb_extract(*, account_id: int, email: str, access_token: str, link_type: str, cdk: str, trigger: str, nonce: str, djb_config=None) -> dict:
    ...
    try:
        ...
        if djb_config is None:
            raise RuntimeError("DJB 提链配置快照缺失")
        job = _create_extract_job(
            token=access_token, email=email, link_type=link_type, cdk=djb_config.card_code,
            backend="djbnb", djb_config=djb_config,
        )
        job_id = str(djb_client._task_id(job) or job.get("id") or "").strip()
        if not job_id:
        ...
        deadline = time.time() + djb_client.task_timeout_seconds(djb_config)
        while True:
            if time.time() >= deadline:
                raise RuntimeError("DJB 提链任务超时")
            snapshot = djb_client.get_task(job_id, task_config=djb_config)
            payload = djb_client.task_payload(snapshot)
            item = _djb_item_for_email(snapshot, email)
            item_status = djb_client.normalized_status(item.get("status") or payload.get("status") or "running")
            progress = payload.get("progress") if isinstance(payload, dict) else {}
            if item_status not in djb_client.DJB_TERMINAL_STATUSES:
                task_status = djb_client.normalized_status(payload.get("status") or "running")
                ...
                result_payload = _map_djb_result(item, link_type)
                final = {
                    "ok": True, "status": "success", "job_id": job_id,
                    "link_type": link_type, "result": result_payload,
                    "message": "DJB 提链成功",
                }
                _update_extract(account_id, nonce, final)
                ...
                reason = str(item.get("error") or payload.get("error") or payload.get("message") or f"DJB 任务状态：{item_status}")[:500]
                raise RuntimeError(reason)
            remaining = max(0.0, deadline - time.time())
            if remaining <= 0:
                raise RuntimeError("DJB 提链任务超时")
            time.sleep(min(djb_client.poll_interval_seconds(djb_config), remaining))
    except Exception as exc:
...
def enqueue_account_extract(*, account_id: int, email: str, access_token: str, trigger: str = "manual", link_type: str | None = None, cdk: str | None = None) -> dict:
    ...
    try:
        ...
        claimed_nonce = str(nonce)
        try:
            fut = _EXECUTOR.submit(
                _run_extract,
                account_id=account_id, email=email, access_token=access_token,
                link_type=lt, cdk=code, trigger=trigger, nonce=claimed_nonce,
            ...
            rollback = {
                ...
                "status": "failed",
                "checked_at": datetime.now().isoformat(timespec="seconds"),
                "error": f"提链任务入队失败: {type(exc).__name__}: {_format_failure_reason(exc)}",
                "message": "提链任务入队失败",
            }
            try:
                db.update_account_extract(account_id, rollback, nonce=claimed_nonce)
            except Exception:
                logger.exception("[提链] 入队失败状态写入异常: account_id=%s", account_id)
            raise

        # A Future canceled while still waiting in the executor never enters
        # _run_extract, so its normal finally block cannot release the slot.
        # If it actually starts, Future.cancel() returns False and the worker
        # remains the sole owner of the release.
        def _release_cancelled_extract(future) -> None:
            if future.cancelled():
                _QUEUE_SLOTS.release()

        fut.add_done_callback(_release_cancelled_extract)
