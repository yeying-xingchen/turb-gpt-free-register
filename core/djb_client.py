...
DJB_TERMINAL_STATUSES = {"success", "plus_success", "failed", "timed_out", "canceled"}
...
def capture_config(*, card_code: str | None = None) -> DjbTaskConfig:
    ...
    code = str(card_code or _runtime_setting("DJB_CARD_CODE", "") or "").strip()
    if not code:
...
def task_payload(snapshot: dict[str, Any]) -> dict[str, Any]:
    """解包官方任务快照可能使用的 data/task envelope。"""
    if not isinstance(snapshot, dict):
        return {}
    for key in ("data", "task"):
        nested = snapshot.get(key)
        if isinstance(nested, dict) and any(k in nested for k in ("status", "items", "progress", "id", "taskId")):
            return nested
    return snapshot
