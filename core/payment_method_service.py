...
def _run_payment_check(*, account_id: int, email: str, access_token: str, trigger: str, regions: list[str] | None = None) -> dict:
    try:
        if not db.mark_account_payment_method_check_running(account_id):
        ...
        db.update_account_payment_method_check(acc_id=account_id, result=result)
        if result.get("ok"):
...
def enqueue_account_payment_method_check(*, account_id: int, email: str, access_token: str, trigger: str = "manual", regions: list[str] | None = None) -> dict:
    ...
    if not db.claim_account_payment_method_check(int(account_id), trigger=trigger):
        _QUEUE_SLOTS.release()
    ...
    try:
        _EXECUTOR.submit(
            ...
            trigger=str(trigger or "manual"), regions=requested,
        )
    except Exception as exc:
        _QUEUE_SLOTS.release()
        ...
        db.update_account_payment_method_check(acc_id=int(account_id), result=result)
