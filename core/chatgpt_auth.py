...
def _ensure_authorize_context(authorize_url: str, session: BrowserSession, email: str) -> str:
    ...
    try:
        ...
        navigator_language = getattr(session, "navigator_language", None)
        try:
            ui_locale = str(navigator_language() or "").strip() if callable(navigator_language) else ""
        except Exception:
            ui_locale = ""
        required = {
